...
#source("C:/Users/chloe/Documents/Research-2015/simulateFactorModel.v2.r")
...
simulateFactorModel = function(n=10, r=2, T=100, fac.err.mean = rep(0, r), fac.err.var = diag(r), Phi = list(matrix(c(0.7, 0, 0, -0.5),ncol=r)), Y.start = c(5, 1), obs.err.mean = rep(0, n), obs.err.var = diag(n), Lambda) {    
	# the order of VAR
	s = length(Phi)

	fac.err = rmvnorm(T, mean=fac.err.mean, sigma=fac.err.var) # T x r
    ...
    #- True Lambda -
    obs.err = rmvnorm(T, mean=obs.err.mean, sigma=obs.err.var)  # T x n
...
use.simulateFactorModel <- function() {
  n=6
  r=2
  T=100 
  ...
  fac.mod = simulateFactorModel(n=n, r=r, T=T, fac.err.mean = fac.err.mean, fac.err.var = fac.err.var, Phi = Phi.true, Y.start = Y.start, obs.err.mean = obs.err.mean, obs.err.var = obs.err.var, Lambda.true)
